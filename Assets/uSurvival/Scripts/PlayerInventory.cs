...
namespace uSurvival
{
    ...
    public partial class PlayerInventory : Inventory
    {
        ...
        public void CmdInventoryMerge(int fromIndex, int toIndex)
        {
                ...
                fromIndex != toIndex)
            {
                ...
                if (slotFrom.amount > 0 && slotTo.amount > 0)
                {
                    ...
                    if (slotFrom.item.data.Equals(slotTo.item.data))
                    {
                        // merge from -> to
        ...
        [Server]
        public void DropItem(Item item, ushort amount)
        {
            ...
            //gff
            drop.ownerTime = NetworkTime.time + ownerDurationTime;
            drop.destroy = true;
        ...
        {
            for (int i = 0; i < slots.Count; ++i)
                if (slots[i].amount > 0 && !slots[i].item.binding && slots[i].item.data.dropFromInventory && player.CheckZoneForDropItems())
        ...
        public void OnRespawn()
        {
            ...
            for (int i = 0; i < slots.Count; ++i)
            {
                if (slots[i].amount == 0)
                {
                    if (i < defaultItems.Length) slots[i] = new ItemSlot(new Item(defaultItems[i].item), defaultItems[i].amount);
                    else slots[i] = new ItemSlot();
                }
            }
        }
        ...
        void OnDragAndDrop_InventorySlot_InventorySlot(int[] slotIndices)
        {
            ...
            if (slots[slotIndices[0]].amount > 0 && slots[slotIndices[1]].amount > 0 &&
                slots[slotIndices[0]].item.data.Equals(slots[slotIndices[1]].item.data))
            {
