...
namespace uSurvival
{
    public abstract partial class Inventory : ItemContainer
    {
        ...
        public int Count(Item item)
        {
            ...
            int amount = 0;
            foreach (ItemSlot slot in slots)
                if (slot.amount > 0 && slot.item.data.Equals(item.data))
                    amount += slot.amount;
            return amount;
        }
        ...
        public bool CanAdd(Item item, int amount)
        {
            ...
            for (int i = 0; i < slots.Count; ++i)
            {
                ...
                // note: .Equals because name AND dynamic variables matter (petLevel etc.)
                else if (slots[i].item.data.Equals(item.data) && CheckBackPackDurability(i))
