...
namespace uSurvival
{
    public abstract partial class Energy : NetworkBehaviour
    {
        ...
        [SyncVar(hook = nameof(OnValueChanged))] short _current = 0;
        private void OnValueChanged(short oldValue, short newValue)
        {
        ...
        // current value
        // set & get: keep between min and max
        //[SyncVar] short _current = 0;
        public short current
        {
            get { return (short)Mathf.Min(_current, max); }
            set
            {
                //gff
                if (entity != null && entity is Player player && player.isGameMaster && player.gameMasterTool.immortality)
                {
                    _current = max;
                    return;
                }
                ...
                _current = (short)Mathf.Clamp(value, 0, max);
        ...
        public abstract short recoveryPerTick { get; }
        ...
        public void Recover()
        {
            if (enabled && health.current > 0)
            {
                ...
                short next = (short)(current + recoveryPerTick);
                ...
                // (if next is bigger max then diff to max is 'next-max')
                    ...
                    overflowInto.current += (short)(next - max);
            }
        }
    }
}