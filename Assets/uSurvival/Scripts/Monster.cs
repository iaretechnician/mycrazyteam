...
namespace uSurvival
{
    ...
    #pragma warning restore CS0109 // member does not hide accessible member
        ...
        [Header("Gff skills")]
        public bool warpToTarget;
        public bool summonTarget;
        public float _warpOrSummonCooldown = 0;
        public float warpOrSummonCooldown = 30;
        ...
        void Update()
        {
            ...
            if (IsWorthUpdating())
            {
                ...
                if (isServer)
                {
                    ...
                    //gff skills
                    //if (_warpOrSummonCooldown > 0)
                    //{
                    //    _warpOrSummonCooldown -= Time.deltaTime;
                    //}
                }
            }
        }
        ...
        bool EventTargetDisappeared() =>
            target == null || (target is Player player && player.currentSafeZoneSource != null);

        bool EventTargetDied() =>
            (target != null && (target.health.current == 0 || (target is Player player && player.currentSafeZoneSource != null)));

        bool EventTargetTooFarToAttack() =>
            target != null && Utils.ClosestDistance(collider, target.collider) > attackRange;

        bool EventTargetTooFarToFollow() =>
            target != null && Vector3.Distance(startPosition, target.collider.ClosestPoint(transform.position)) > followDistance;
        ...
        {
            return entity is Player player && player.currentSafeZoneSource == null && !player.isCreatedCharacter && player.netIdentity.visible != Visibility.ForceHidden && 
        ...
        public void OnAggro(Entity entity)
        {
            ...
            {
                //Debug.Log("Set target:" + entity.name + " For " + name);
        ...
        {
            // play sound (if any)
            if (attackSounds != null && attackSounds.Length > 0) audioSource.PlayOneShot(attackSounds[Random.Range(0, attackSounds.Length)]);
        }

        private void CheckSkill()
        {
            if (target is Player pl && _warpOrSummonCooldown <= 0)
            {
                if (summonTarget && Random.Range(0f, 1f) > 0.5f)
                {
                    Debug.Log("warp player to monster");
                    _warpOrSummonCooldown = warpOrSummonCooldown;
                    pl.movement.Warp(transform.position);
                }
                else if (agent.hasPath && warpToTarget && Random.Range(0f, 1f) > 0.5f)
                {
                    _warpOrSummonCooldown = warpOrSummonCooldown;
