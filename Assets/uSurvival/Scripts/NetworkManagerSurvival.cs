...
namespace uSurvival
{
    ...
    public partial class NetworkManagerSurvival : NetworkManager
    {
        ...
        {
            new ServerInfo{name="Local", ip="127.0.0.1", port = 7777}
        };

        public UILoginExtendedAutorization loginAuth;
        private string directoryPath = "";
        private string filePath = "";

        //Added by ConveyorXDev
        public void SaveServersList()
        {
            ...
            {
                //name ip port
                data += "\n" + serversList[i].name + " " + serversList[i].ip + " " + serversList[i].port.ToString();
            }
        ...
        public void LoadServersList()
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    if (info.Length == 3)
                    {
                        ushort.TryParse(info[2], out ushort port_);
                        serversList.Add(new ServerInfo { name = info[0], ip = info[1], port = port_ });
                    }
                }
            }
            else
            {
                //find local ip then start at that one!
                //serversList.Add(new ServerInfo { ip = GetIP(), name = "local-pc", port = 7777 });
                serversList.Add(new ServerInfo { ip = "kcwin.iaregamer.com", name = "remote_test", port = 7777 });
            }

            loginAuth.PopulateServerDropDown();
        }

        public void AddCurrentConnectedServerToList()
        {
            //add current server if connected to in server list if not already added!
            bool exsists = false;
            for (int i = 0; i < serversList.Count; i++)
            {
                if (serversList[i].ip == networkAddress && serversList[i].port == kcpTransport.port)
                {
                    exsists = true;
                }
            }
            ...
            if (!exsists)
            {
                serversList.Add(new ServerInfo { ip = networkAddress, port = kcpTransport.port, name = "server"+Random.Range(0, 9999).ToString("0000") });
            }
        }
        ...
        public void ReadPort(string value) 
        {
            ushort.TryParse(value, out kcpTransport.port);
        }
        ...
        public void Reconnnect()
        {
            ...
            Debug.Log("Reconnecting to server on new IP and port " + networkAddress + ":"+kcpTransport.port);
        ...
        {
            networkAddress = serversList[i].ip;
            kcpTransport.port = serversList[i].port;
            Reconnnect();
        }
        ...
        public int characterNameMinLength = 6;
        ...
        public bool IsAllowedCharacterName(string characterName)
        {
            ...
            return characterName.Length <= characterNameMaxLength && characterName.Length >= characterNameMinLength &&
                   Regex.IsMatch(characterName, @"^[a-za-яA-ZА-Ю0-9_]+$");
        }
        ...
        public override void Awake()
        {
            ...
            //load server list!
            directoryPath = Application.persistentDataPath + "/Servers/";
            filePath = Application.persistentDataPath + "/Servers/servers_list.cfg";
            LoadServersList();
        ...
        public override void Update()
        {
            ...
            if (NetworkClient.isConnected)
            {
                infoText.text = "Connection to Server = " + networkAddress + ":" + kcpTransport.port;
            }
            else 
            {
                infoText.text = "Not connected to any server!";
            }
        }
        ...
        public override void OnStartServer()
        {
            ...
            //gff
            OnStartServer_Registration();
            //OnStartServer_RegistrationVkPlay();
            ...
            Debug.Log($"[CX]Started Server at {networkAddress}:{kcpTransport.port}");
        ...
        {
            Debug.Log("[CX] Server Stopped!");
            CancelInvoke(nameof(Save));

            if (headlessStartMode == HeadlessStartOptions.AutoStartServer && !quitting)
            {
                //automatically restart the server
                Debug.Log("Restarting server in 3 seconds...");
                Invoke(nameof(RestartServer), 3f);
            }
        }
        ...
        public override void OnClientConnect()
        {
            ...
            //this means the server player connected to was successful (add this server to list of servers for future connections!)
            AddCurrentConnectedServerToList();
        }
        ...
        Player CreateCharacter(GameObject classPrefab, string characterName, string account)
        {
            ...
            player.transform.position = GetRespawnPosition((int)player.selectedClan);
                    ...
                    if (!Database.singleton.CharacterExists(message.name))
                    {
                        ...
                        if (Database.singleton.CharactersForAccount(account).Count < characterLimit)
                        {
                            ...
                            if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                            {
                                ...
                                //gff customization addon
                                OnServerCharacterCreate_Customization(message, player);
                                OnServerCharacterCreate_NpcGroups(player);

                                //gff
                                player.itemMall.coins = Database.singleton.LoadCoinsForAccount(player.account);
                                Database.singleton.CharacterLoad_Storage(player);
                                Database.singleton.CharacterLoad_WeaponSkins(player);
        ...
        void Save()
        {
            ...
            Database.singleton.SaveAllGuilds();
        }
        ...
        public override void OnClientDisconnect()
        {
            ...
            loginAuth.panel.SetActive(true);
        }
