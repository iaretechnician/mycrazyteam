...
namespace uSurvival
{
    ...
    public partial class ItemDrop : NetworkBehaviour, Interactable
    {
        [Header("Item")]
    ...
    #pragma warning restore CS0649 // Field is never assigned to
        ...
        [SyncVar] public ushort amount = 1; // sometimes set on server, needs to sync
        ...
        public string InteractionText()
        {
            if (Player.localPlayer != null && itemData != null && amount > 0)
                return amount > 1 ? Localization.Translate(item.name) + " x " + amount : Localization.Translate(item.name);
            return "";
        }
        ...
        {
            player.audioSource.PlayOneShot(itemData.lootSound);
        }
