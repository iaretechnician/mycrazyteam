...
namespace GFFAddons
{
    public static partial class PartySystem
    {
        ...
        public static void ChangeLeader(int partyId, string requester, string member)
        {
            // party exists?
            Party party;
            if (parties.TryGetValue(partyId, out party))
            {
                if (party.master == requester && party.Contains(member))
                {
                    //find pos in array new party master
                    int pos = 0;
                    for (int i = 0; i < party.members.Length; i++)
                    {
                        if (party.members[i] == member) pos = i;
                    }

                    Debug.Log("before : master is " + party.members[0] + " / " + party.members[pos]);

                    party.members[0] = member;
                    party.members[pos] = requester;

                    Debug.Log("after : master is " + party.members[0] + " / " + party.members[pos]);
