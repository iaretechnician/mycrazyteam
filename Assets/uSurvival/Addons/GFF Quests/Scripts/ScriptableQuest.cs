...
namespace GFFAddons
{
    public abstract partial class ScriptableQuest : ScriptableObject
    {
        [Header("General")]
        ...
        public ushort rewardCoins;
        public ScriptableItemAndAmount[] rewardItems;
        public bool chooseReward;

        [Header("GFF Quest Type")]
        [Tooltip("minutes")] public short cooldown = 0;
        ...
        [Header("Task by Language")]
        [SerializeField] private LocalizeText[] taskLocalize;
        public string GetTaskByLanguage(SystemLanguage lang)
        {
            for (int i = 0; i < taskLocalize.Length; i++)
            {
                if (taskLocalize[i].language == lang) return taskLocalize[i].description;
            }
            return null;
        }
        ...
        */
        ...
        private void OnValidate()
        {
            for (int i = 0; i < descriptionLocalize.Length; i++)
            {
                if (descriptionLocalize[i].text != descriptionLocalize[i].language.ToString()) descriptionLocalize[i].text = descriptionLocalize[i].language.ToString();
            }

            for (int i = 0; i < taskLocalize.Length; i++)
            {
                if (taskLocalize[i].text != taskLocalize[i].language.ToString()) taskLocalize[i].text = taskLocalize[i].language.ToString();
            }

            for (int i = 0;i < rewardItems.Length; i++)
            {
                if (rewardItems[i].item != null) rewardItems[i].name = rewardItems[i].item.name;
                else rewardItems[i].name = "null";
            }
        }
#endif
    }
